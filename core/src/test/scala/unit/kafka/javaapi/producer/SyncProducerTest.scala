...
class SyncProducerTest extends JUnitSuite {
  ...
  def testUnreachableServer() {
    ...
    try {
      producer.send("test", 0, new ByteBufferMessageSet(false, getMessageList(new Message(messageBytes))))
    }catch {
  ...
  def testReachableServer() {
    ...
    try {
      producer.multiSend(Array(new ProducerRequest("test", 0,
        new ByteBufferMessageSet(false, getMessageList(new Message(messageBytes))))))
    }catch {
      case e: Exception => failed=true
    }
    Assert.assertFalse(failed)
