...
class ProducerTest extends JUnitSuite {
  ...
  def setUp() {
    ...
    producer1.send("test-topic", new ByteBufferMessageSet(false, new Message("test".getBytes())))

    producer2 = new SyncProducer(new SyncProducerConfig(props) {
      override val port = port2
    })
    producer2.send("test-topic", new ByteBufferMessageSet(false, new Message("test".getBytes())))
  ...
  def testSend() {
    ...
    syncProducer2.send(topic, 0, new ByteBufferMessageSet(false, new Message("test1".getBytes)))
  ...
  def testSendSingleMessage() {
    ...
    syncProducer1.send(topic, -1, new ByteBufferMessageSet(false, new Message("t".getBytes())))
    EasyMock.expectLastCall
    syncProducer1.close
    EasyMock.expectLastCall
    EasyMock.replay(syncProducer1)
  ...
  def testSyncProducerPool() {
    ...
    val syncProducer1 = EasyMock.createMock(classOf[SyncProducer])
    val syncProducer2 = EasyMock.createMock(classOf[SyncProducer])
    syncProducer1.send("test-topic", 0, new ByteBufferMessageSet(false, new Message("test1".getBytes)))
    EasyMock.expectLastCall
    syncProducer1.close
    EasyMock.expectLastCall
    syncProducer2.close
  ...
  def testPartitionedSendToNewTopic() {
    ...
    val syncProducer1 = EasyMock.createMock(classOf[SyncProducer])
    val syncProducer2 = EasyMock.createMock(classOf[SyncProducer])
    syncProducer1.send("test-topic1", 0, new ByteBufferMessageSet(false, new Message("test1".getBytes)))
    EasyMock.expectLastCall
    syncProducer1.send("test-topic1", 0, new ByteBufferMessageSet(false, new Message("test1".getBytes)))
    EasyMock.expectLastCall
    syncProducer1.close
    ...
    producer1.send("test-topic1", new ByteBufferMessageSet(false, new Message("test".getBytes())))
    Thread.sleep(100)
  ...
  def testPartitionedSendToNewBrokerInExistingTopic() {
    ...
    syncProducer3.send("test-topic", 2, new ByteBufferMessageSet(false, new Message("test1".getBytes)))
    EasyMock.expectLastCall
    syncProducer1.close
    EasyMock.expectLastCall
    syncProducer2.close
    ...
    tempProducer.send("test-topic", new ByteBufferMessageSet(false, new Message("test".getBytes())))
