...
class LogManagerTest extends TestCase {
  ...
  def testTimeBasedFlush() {
    ...
    config = new KafkaConfig(props) {
                   ...
                   override val flushIntervalMap = Utils.getTopicFlushIntervals("timebasedflush:500")
                 }
    logManager = new LogManager(config, null, time, -1, maxLogAge)
    logManager.startup
    Thread.sleep(1000)
    ...
    assertTrue("The last flush time has to be within defaultflushInterval of current time ",
                     (System.currentTimeMillis - log.getLastFlushedTime) < 600)
  }
  ...
  def testConfigurablePartitions() {
    ...
    config = new KafkaConfig(props) {
                   override val logFileSize = 1024
                   override val enableZookeeper = false
                   override val topicPartitionsMap = Utils.getTopicPartitions("testPartition:4")
                 }
    logManager = new LogManager(config, null, time, -1, maxLogAge)
    logManager.startup
    for(i <- 0 until 4) {
    ...
    try
    {
      val log = logManager.getOrCreateLog("testPartition", 4)
