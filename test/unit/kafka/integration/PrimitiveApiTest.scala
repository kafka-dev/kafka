...
class PrimitiveApiTest extends TestCase with ProducerConsumerTestHarness with KafkaServerTestHarness {
  ...
  def testProduceAndFetch() {
    ...
    val sent2 = new ByteBufferMessageSet(false, new java.util.ArrayList[Message]())
    producer.send(topic, sent2)
    Thread.sleep(200)
    sent2.serialized.rewind
    var fetched2 = consumer.fetch(topic, 0, 10000)
