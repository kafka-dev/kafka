...
class LogManager(val config: KafkaConfig,
  ...
  private val logFlashIntervalMap = config.flushIntervalMap
  ...
  def startup() {
    ...
    logger.info("starting log flusher every " + config.flushSchedulerThreadRate + " ms")    
  ...
  private def flushAllLogs() = {
    ...
    for (log <- getLogIterator)
    {
      ...
      if(logFlashIntervalMap.contains(log.getTopicName))
        logFlushInterval = logFlashIntervalMap(log.getTopicName)
