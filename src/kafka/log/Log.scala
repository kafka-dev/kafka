...
object Log {
  ...
  def findRange[T <: Range](ranges: Array[T], value: Long, arraySize: Int): Option[T] = {
    ...
    while(low <= high) {
      ...
      if(value >= found.start && value <= found.end)
        return Some(found)
      else if(value > found.end)
        low = mid + 1
      else
        high = mid - 1
    }
...
class LogSegment(val file: File, val messageSet: FileMessageSet, val start: Long) extends Range {
  ...
  override def toString() = "(" + file + ", " + start + ", " + end + ")"
}
...
class Log(val dir: File, val maxSize: Long, val flushInterval: Int) {
  ...
  def roll() {
    lock synchronized {
      ...
      flush()
      var newOffset: Long = 0
      if(!(last.start == 0 && last.messageSet.highWaterMark == 0)) {
        newOffset = last.end + 1L
      }
